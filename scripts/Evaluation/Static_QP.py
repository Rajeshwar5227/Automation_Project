...
class StaticQpEvaluation:
    def __init__(self):
        print()

    def static_qp_evaluation(self):
        # "X-APPLMA": "true", "APP-NAME": "py3app"
        # host_ip = '35.154.36.218'  # Master DB
        # # host_ip = '35.154.213.175'    #Replica DB
        # db_name = "qa_testing"  # new db name is "qa_testing"
        # login_name = "qauser"
        # pwd = "qauser"
        ...
        self.userName = "admin"
        self.inputFile = r"D:\Automation\API_Automation\Input\staticQP_Evaluation.xls"
        self.outputFile = r"D:\Automation\API_Automation\Output\StaticQP_Evaluation_Check.xls"
        self.xlsSheetName = "staticQP_Evaluation"
        # --------------------------------------------------------------------------------------------------------------
        ...
        ws = wb_result.add_sheet(self.xlsSheetName)
        ...
        while n < sh1.nrows:
            ...
            # ----------------------------------------------------------------------------------------------------------
            login_to_test_header = {"content-type": "application/json", "X-APPLMA": self.appLAM, "APP-NAME": self.appName}
            ...
            # ----------------------------------------------------------------------------------------------------------
            submitTestResult_header = {"content-type": "application/json", "X-APPLMA": self.appLAM, "APP-NAME": self.appName,
            ...
            #  Login to AMS
            # ----------------------------------------------------------------------------------------------------------
            login_user_header = {"content-type": "application/json", "X-APPLMA": self.appLAM, "APP-NAME": self.appName}
            login_user_data = {"LoginName": self.loginName, "Password": self.password, "TenantAlias": self.tenantAlias,
            ...
            try:
                ...
                # conn = mysql.connector.connect(host='35.154.213.175',
                ...
                test_user_id1 = list(test_user_id)
                cursor.execute(
                    f"select question_id, obtained_marks from test_results where testuser_id = {test_user_id1[0]};")
                        ...
                        if (question_ids[j] == data[i][0]):
                            if (rows[Expectedcell] == data[i][1]):
                                ...
                                print("Question -- Candidate_Id - ", candidateId, "Question_Id - ", question_ids[j],
                                      "Expected_Marks - ", rows[Expectedcell], "Actual_Marks - ", data[i][1])
                            break
                        i += 1
                    j += 1
                    col += 2
            ...
            responses = json.loads(view_candidate_score_by_candidate_id_request.content)
            print(responses)
            # testUserScore = len(json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks'])
            # jsonPrint = json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks']
            # print("jsonPrint", jsonPrint)
            # for ite in range(0, testUserScore):
            #     if candidate Id == candidateId:
            k = 0
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][0]['id'] == 12857:
            ...
            k = 0
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks'][0]['sectionInfo'][k]["sectionId"] == 46869:
                    Actual_Section1_Score = json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks'][0]['sectionInfo'][k]['obtainedMarks']
            ...
            k = 0
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks'][0]['sectionInfo'][k]["sectionId"] == 46870:
                    Actual_Section2_Score = \
                        json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][0]['sectionInfo'][k]['obtainedMarks']
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][0]['sectionInfo'][k]["sectionId"] == 46871:
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][1]['id'] == 12858:
                    Actual_Group2_Score = \
                        json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][1]['obtainedMarks']
            ...
            k = 0
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks'][1]['sectionInfo'][k]["sectionId"] == 46872:
                    Actual_Section4_Score = json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][1]['sectionInfo'][k]['obtainedMarks']
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][1]['sectionInfo'][k]["sectionId"] == 46873:
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][2]['id'] == 12859:
                    Actual_Group3_Score = json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][2]['obtainedMarks']
                    if round(Expected_Grp_3_Total, 3) == round(Actual_Group3_Score, 3):
                        ...
                        print("Group -- Candidate_Id - ", candidateId, " group_1_id - ", group_3_id,
                              "Expected Score - ", Expected_Grp_3_Total, "Actual Score - ",
                              Actual_Group3_Score)
                    break
                k += 1
            k = 0
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][2]['sectionInfo'][k]["sectionId"] == 46874:
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                            'groupAndSectionWiseMarks'][2]['sectionInfo'][k]["sectionId"] == 46875:
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                    'groupAndSectionWiseMarks'][3]['id'] == 12860:
                    Actual_Group4_Score = json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
            ...
            while k <= 13:
                if \
                json.loads(view_candidate_score_by_candidate_id_request.content)['data']['groupAndSectionWiseMarks'][3][
                    'sectionInfo'][k]["sectionId"] == 46876:
                    Actual_Section9_Score = json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
            ...
            while k <= 13:
                if json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                    'groupAndSectionWiseMarks'][3]['sectionInfo'][k]["sectionId"] == 46877:
                    Actual_Section8_Score = json.loads(view_candidate_score_by_candidate_id_request.content)['data'][
                        'groupAndSectionWiseMarks'][3]['sectionInfo'][k]['obtainedMarks']
            ...
            wb_result.save(self.outputFile)
            n += 1
            rownum += 1
