...
class McqwTimeSpentPerQuestion:
    def __init__(self):
        self.overall_Status = []
        ...
        # self.inputFilePath = input_dir / 'MCQW_Candidate_Performance_TSPQ.xls'
        # self.outputFilePath = output_dir / 'MCQW_Candidate_Performance_TSPQ.html'
        self.inputFilePath = r"D:\Automation\API_Automation\Input\MCQW_Candidate_Performance_TSPQ.xls"
        self.outputFilePath = r"D:\Automation\API_Automation\Output\MCQW_Candidate_Performance_TSPQ.html"
    ...
    def mcqw_time_spent_per_question(self):
        ...
        self.file.write("""<html>
                ...
                <div class="div-h2">
                    <h1>MCQW Deep Dive - Candidate Performance By Time Spent Per Question</h1>
                </div>
                </head>
    ...
    def reportData(self):
        ...
        for i in range(len(transcript_response['data']['mcqWithWeightage'])):
            question_ids.append(transcript_response['data']['mcqWithWeightage'][i]['id'])
            qid = transcript_response['data']['mcqWithWeightage'][i]['id']
            candidateAnswer = transcript_response['data']['mcqWithWeightage'][i]['candidateAnswer']
            obtainedMark = transcript_response['data']['mcqWithWeightage'][i]['obtainedMark']
            timeSpent = transcript_response['data']['mcqWithWeightage'][i]['timeSpent']
            difficultyLevel = transcript_response['data']['mcqWithWeightage'][i]['difficultyLevel']
            question_mark = transcript_response['data']['mcqWithWeightage'][i]['mark']
            ...
            if (candidateAnswer is None and obtainedMark == 0 and timeSpent == 0) or (
                ...
                status = "Wrong answer"
            elif candidateAnswer is not None and (obtainedMark == question_mark or obtainedMark < 0) and timeSpent > 0:
                status = "Correct answer"
            elif candidateAnswer is not None and (obtainedMark > 0 and obtainedMark < question_mark or obtainedMark < 0) and timeSpent > 0:
