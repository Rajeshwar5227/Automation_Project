...
class FibComprehensive:
    ...
    def __init__(self):
        ...
        self.inputFilePath = r"D:\Automation\API_Automation\Input\FIB_Comprehensive.xls"
        self.outputFilePath = r"D:\Automation\API_Automation\Output\FIB_Comprehensive.html"
    ...
    def fib_comprehensive(self):
        ...
        self.file.write("""<html>
                ...
                <div class="div-h2">
                    <h1>FIB Deep Dive - Comprehensive Report</h1>
                </div>
                </head>
        ...
        for login_details in self.xls_values:
            ...
            for i in range(0, int((len(self.expected_Data)) / 7)):
                ...
                self.candAns.append(self.expected_Data[j + 5])
                self.candAnsType.append(type(self.expected_Data[j + 5]))
                j += 7
            # print(self.candAns)
            # print(self.candAnsType)
    ...
    def reportData(self):
        ...
        print(transcript_request.headers)
        print("Headers - ", transcript_request.headers)
        testId = transcript_response['data']['assessment']['testId']
        fibQuestions = transcript_response['data']['fillInTheBlank']
        ...
        # find each section in group and list it
        grpVsSec = dict()
        sections = []
        for indx in groupSectionInfo:
            for indxx in indx.get('sectionInfo'):
                if indxx.get('questionType') == "FillInTheBlank":
        ...
        for data in fibQuestions:
            ...
            if data.get('jsonCorrectAnswerStr') is None:
                q_data.append("Skipped")
            else:
                abc = data.get("jsonCorrectAnswer")
                self.correct_Answer = ""
                self.test = list()
                if len(abc) > 1:
                    for k, v in abc.items():
                        self.test.append(v.get("answer"))
                    self.correct_Answer = self.test
                    # self.correct_Answer = list(map(str.strip, self.correct_Answer))
                    q_data.append(self.correct_Answer)
                else:
                    for k, v in abc.items():
                        self.correct_Answer = v.get("answer")
                        q_data.append(self.correct_Answer)
            self.candidate_Answer = ""
            if data.get("jsonCandidateAnswer") is None:
                q_data.append("N/A")
                self.candidate_Answer = "N/A"
            else:
                xyz = data.get("jsonCandidateAnswer")
                self.test1 = list()
                if len(xyz) > 1:
                    for k, v in xyz.items():
                        self.test1.append(v)
                    self.candidate_Answer = self.test1
                    # self.candidate_Answer = list(map(str.strip, self.candidate_Answer))
                    q_data.append(self.candidate_Answer)

                else:
                    for k, v in xyz.items():
                        q_data.append(v)
                        self.candidate_Answer = v

            if type(self.candidate_Answer) is str and type(self.correct_Answer) is str:
                if self.candidate_Answer == "N/A":
                    q_data.append("Skipped")
                elif str(self.correct_Answer).lower() == str(self.candidate_Answer).lower():
                    q_data.append("Correct")
                else:
                    q_data.append("Wrong")
            elif type(self.candidate_Answer) is list and type(self.correct_Answer) is list:
                cand_ans = list()
                for items in self.candidate_Answer:
                    cand_ans.append(items.lower()) # type: ignore
                corr_ans = list()
                for items in self.correct_Answer:
                    corr_ans.append((items.lower())) # type: ignore
                if cand_ans == "N/A":
                    q_data.append("Skipped")
                elif corr_ans == cand_ans:
                    q_data.append("Correct")
                else:
                    q_data.append("Wrong")
            elif type(self.candidate_Answer) is str and type(self.correct_Answer) is list:
                self.correct_Answer = map(str.strip, self.correct_Answer)
                if self.candidate_Answer == "N/A":
                    q_data.append("Skipped")
                elif self.candidate_Answer in self.correct_Answer:
                    q_data.append("Correct")
                else:
                    q_data.append("Wrong")
            else:
                if self.candidate_Answer == "N/A":
                    q_data.append("Skipped")
                elif self.correct_Answer == self.candidate_Answer:
                    q_data.append("Correct")
                else:
                    q_data.append("Wrong")
        ...
        for iter in range(0, len(self.expected_Data)):
            # print(type(self.expected_Data[iter]), self.expected_Data[iter])
            # print(type(actual_Data[iter]), actual_Data[iter])


            if type(actual_Data[iter]) is list:
                actual_Data[iter] = ','.join(str(e) for e in actual_Data[iter])
                actual_Data[iter] = actual_Data[iter].replace(", ", ",")
                # print(type(actual_Data[iter]), actual_Data[iter].replace(", ", ","))
                # actual_Data[iter] = str(actual_Data[iter])

            # else:
                actual_Data[iter] = actual_Data[iter]
            # print(type(actual_Data[iter]), actual_Data[iter])
            # print(type(self.expected_Data[iter]), self.expected_Data[iter])
...
if __name__ == "__main__":
    trans = FibComprehensive()
    trans.fib_comprehensive()
    trans.login()
    trans.reportData()
    trans.logout()