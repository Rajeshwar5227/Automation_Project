...
class McqComprehensive:
    def __init__(self):
        ...
        # self.inputFilePath = input_dir / 'MCQ_Comprehensive.xls'
        # self.outputFilePath = output_dir / 'MCQ_Comprehensive.html'
        self.inputFilePath = r"D:\Automation\API_Automation\Input\MCQ_Comprehensive.xls"
        self.outputFilePath = r"D:\Automation\API_Automation\Output\MCQ_Comprehensive.html"
    ...
    def mcq_comprehensive(self):
        ...
        self.file.write("""<html>
                ...
                <div class="div-h2">
                    <h1>MCQ Deep Dive - Comprehensive Report</h1>
                </div>
                </head>
        ...
        for login_details in self.xls_values:
            ...
            for i in range(0, int((len(self.expected_Data)) / 6)):
                self.expected_question_data[self.expected_Data[j]] = [self.expected_Data[j + 1],
                                                                      self.expected_Data[j + 2],
                                                                      self.expected_Data[j + 3],
                                                                      self.expected_Data[j + 4],
                                                                      self.expected_Data[j + 5]]
                j += 6
    ...
    def reportData(self):
        ...
        mcqQuestions = transcript_response['data']['mcq']
        rtcQuestions = transcript_response['data']['referenceToContext']
        ...
        for indx in groupSectionInfo:
            for indxx in indx.get('sectionInfo'):
                if indxx.get('questionType') == "MCQ" or indxx.get('questionType') == "ReferenceToContext":
        ...
        for data in mcqQuestions:
            ...
            q_data.append(data.get('correctAnswer'))
        ...
        for data1 in rtcQuestions:
            sec_id = data1.get('sectionId')
            if sec_id in idVsSec:
                sec_Name = idVsSec.get(sec_id)
            else:
                print("No")
            ...
            q_data.append(sec_Name)
            q_data.append(grp_Name)
            q_data.append(data1.get('timeSpent'))
            q_data.append(data1.get('correctAnswer'))
            if data1.get('candidateAnswer') is None:
                q_data.append("skipped")
            else:
                q_data.append(data1.get('candidateAnswer'))
            actual_question_data[data1.get('id')] = q_data
            q_data = []
