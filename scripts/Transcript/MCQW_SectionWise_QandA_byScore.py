...
class McqwSectionWiseQandAByScore:
    def __init__(self):
        self.overall_Status = []
        ...
        # self.inputFilePath = input_dir / 'MCQW_SectionWise_QA_byScore.xls'
        # self.outputFilePath = output_dir / 'MCQW_SectionWise_QA_byScore.html'
        self.inputFilePath = r"D:\Automation\API_Automation\Input\MCQW_SectionWise_QA_byScore.xls"
        self.outputFilePath = r"D:\Automation\API_Automation\Output\MCQW_SectionWise_QA_byScore.html"
    ...
    def mcqw_section_wise_q_and_a_by_score(self):
        # --------------------------------------------------------------------------------------------------------------
        # Read from Excel
        # --------------------------------------------------------------------------------------------------------------
        ...
        self.file.write("""<html>
                ...
                <div class="div-h2">
                    <h1>MCQW Deep Dive - Candidate Performance SectionWise_QandA_byScore</h1>
                </div>
                </head>
        ...
        for elem in self.grpVsSec:
            for elemm in groupAndSectionWiseMarks:
                if elem == elemm['name']:
                    ...
                    for elemmm in self.grpVsSec[elem]:
                        for elemmmm in elemm['sectionInfo']:
                            if elemmm == elemmmm['sectionName']:
                                ...
                                if type(elemmmm['obtainedMarks']) is float:
                                    sec_obt_Marks = round(elemmmm['obtainedMarks'], 2)
                                    actual_Data.append(sec_obt_Marks)
                                else:
                                    actual_Data.append(elemmmm['obtainedMarks'])
                                actual_Data.append(elemmmm['totalMarks'])
                                if type(sec_difficultywiseMarks['low']['obtainedMarks']) is float:
                                    sec_low_obt_Marks = round(sec_difficultywiseMarks['low']['obtainedMarks'], 2)
                                    actual_Data.append(sec_low_obt_Marks)
                                else:
                                    actual_Data.append(sec_difficultywiseMarks['low']['obtainedMarks'])
                                actual_Data.append(sec_difficultywiseMarks['low']['maxMarks'])

                                if type(sec_difficultywiseMarks['medium']['obtainedMarks']) is float:
                                    sec_medium_obt_Marks = round(sec_difficultywiseMarks['medium']['obtainedMarks'], 2)
                                    actual_Data.append(sec_medium_obt_Marks)
                                else:
                                ...
                                if type(sec_difficultywiseMarks['high']['obtainedMarks']) is float:
                                    sec_high_obt_Marks = round(sec_difficultywiseMarks['high']['obtainedMarks'], 2)
                                    actual_Data.append(sec_high_obt_Marks)
                                else:
                                    actual_Data.append(sec_difficultywiseMarks['high']['obtainedMarks'])
                                actual_Data.append(sec_difficultywiseMarks['high']['maxMarks'])
                    if type(elemm.get('obtainedMarks')) is float:
                        grp_obt_Marks = round(elemm.get('obtainedMarks'), 2)
                        actual_Data.append(grp_obt_Marks)
                    else:
                        actual_Data.append(elemm.get('obtainedMarks'))
                    # actual_Data.append(elemm.get('obtainedMarks'))
                    actual_Data.append(elemm.get('totalMarks'))
                    if type(grp_difficultywiseMarks['low']['obtainedMarks']) is float:
                        grp_low_obt_Marks = round(grp_difficultywiseMarks['low']['obtainedMarks'], 2)
                        actual_Data.append(grp_low_obt_Marks)
                    else:
                        actual_Data.append(grp_difficultywiseMarks['low']['obtainedMarks'])
                    # actual_Data.append(grp_difficultywiseMarks['low']['obtainedMarks'])
                    actual_Data.append(grp_difficultywiseMarks['low']['maxMarks'])
                    if type(grp_difficultywiseMarks['medium']['obtainedMarks']) is float:
                        grp_medium_obt_Marks = round(grp_difficultywiseMarks['medium']['obtainedMarks'], 2)
                        actual_Data.append(grp_medium_obt_Marks)
                    else:
                    ...
                    if type(grp_difficultywiseMarks['high']['obtainedMarks']) is float:
                        grp_high_medium_obt_Marks = round(grp_difficultywiseMarks['high']['obtainedMarks'], 2)
                        actual_Data.append(grp_high_medium_obt_Marks)
                    else:
