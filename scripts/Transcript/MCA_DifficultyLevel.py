...
class McaDifficultyLevel:
    def __init__(self):
        ...
        self.inputFilePath = r"D:\Automation\API_Automation\Input\MCA_Candidate_Performance_DL.xls"
        self.outputFilePath = r"D:\Automation\API_Automation\Output\MCA_Candidate_Performance_DL.html"
        self.outputSheetName = "MCA_DL_Transcript"
        # --------------------------------------------------------------------------------------------------------------
    ...
    def mca_difficulty_level(self):
        ...
        self.file.write("""<html>
                ...
                <div class="div-h2">
                    <h1>MCA Deep Dive - Candidate Performance By Difficulty Level</h1>
                </div>
                </head>
    ...
    def reportData(self):
        ...
        MCA_question_Count = len(transcript_response['data']['multipleCorrectAnswer'])
        Total_question_Count = MCA_question_Count
        ...
        mca_Low_Correct = 0
        mca_Low_Skipped = 0
        mca_High_Incorrect = 0
        mca_High_Correct = 0
        mca_High_Skipped = 0
        mca_Medium_Incorret = 0
        mca_Medium_Correct = 0
        mca_Medium_Skipped = 0
        for i in range(len(groupAndSectionWiseMarks)):
            for j in range(len(groupAndSectionWiseMarks[i]['sectionInfo'])):
                if groupAndSectionWiseMarks[i]['sectionInfo'][j]['questionType'] == "MultipleCorrectAnswer" or \
                    ...
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['low'][
                        ...
                        mca_Low_Incorrect = mca_Low_Incorrect + \
                                            groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['low'][
                                                'inCorrect']
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['low'][
                        'correct'] is not None:
                        mca_Low_Correct = mca_Low_Correct + \
                                          groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['low'][
                                              'correct']
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['low'][
                        'skipped'] is not None:
                        mca_Low_Skipped = mca_Low_Skipped + \
                                          groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['low'][
                                              'skipped']

                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['medium'][
                        'inCorrect'] is not None:
                        mca_Medium_Incorret = mca_Medium_Incorret + \
                                              groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount'][
                                                  'medium']['inCorrect']
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['medium'][
                        'correct'] is not None:
                        mca_Medium_Correct = mca_Medium_Correct + \
                                             groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount'][
                                                 'medium']['correct']
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['medium'][
                        'skipped'] is not None:
                        mca_Medium_Skipped = mca_Medium_Skipped + \
                                             groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount'][
                                                 'medium']['skipped']

                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['high'][
                        'inCorrect'] is not None:
                        mca_High_Incorrect = mca_High_Incorrect + \
                                             groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount'][
                                                 'high']['inCorrect']
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['high'][
                        'correct'] is not None:
                        mca_High_Correct = mca_High_Correct + \
                                           groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['high'][
                                               'correct']
                    if groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['high'][
                        'skipped'] is not None:
                        mca_High_Skipped = mca_High_Skipped + \
                                           groupAndSectionWiseMarks[i]['sectionInfo'][j]['difficultWiseCount']['high'][
                                               'skipped']

            mca_CIS['Low'] = [mca_Low_Correct, mca_Low_Incorrect, mca_Low_Skipped]
            mca_CIS['Medium'] = [mca_Medium_Correct, mca_Medium_Incorret, mca_Medium_Skipped]
            mca_CIS['High'] = [mca_High_Correct, mca_High_Incorrect, mca_High_Skipped]

        print("Actual : ", mca_CIS)
        ...
        for i in self.input_Data:
            if i in mca_CIS:
                ...
                self.status.append("Pass")
                for j in range(len(mca_CIS[i])):
                    if self.input_Data[i][j] == mca_CIS[i][j]:
                        self.file.write("""<td class="td-pass">""" + str(mca_CIS[i][j]) + """</td>""")
                        self.status.append("Pass")
                    else:
                        self.file.write("""<td class="td-fail">""" + str(mca_CIS[i][j]) + """</td>""")
                        self.status.append("Fail")
            else:
                self.file.write("""<td class="td-pass">""" + str(i) + """</td>""")
                self.status.append("Pass")
