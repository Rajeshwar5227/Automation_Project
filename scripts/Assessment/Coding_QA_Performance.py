...
class CodingQAPerformance(login.CommonLogin):
    ...
    def __init__(self):
        ...
        self.inputFilePath = inputFile.assessment['Coding_QAPerformance']
        self.outputFilePath = outputFile.OUTPUT_PATH['Coding_QAPerformance']
        self.outputSheetName = "Coding_QAPerformance_Transcript"
        # --------------------------------------------------------------------------------------------------------------
    ...
    def coding_qa_performance(self, server):
        ...
        self.file.write("""<html>
                ...
                <div class="div-h2">
                    <h1>Candidate Transcript - Coding_QAPerformance</h1>
                </div>
                </head>
        ...
        for login_details in self.xls_values:
            ...
            j = 2
            totalQue = int((len(self.expected_Data)) / 10)
            for i in range(0, int((len(self.expected_Data)) / 10)):
                self.expected_question_data[self.expected_Data[j]] = [self.expected_Data[j + 1], self.expected_Data[j + 2], self.expected_Data[j + 3], self.expected_Data[j + 4], self.expected_Data[j + 5], self.expected_Data[j + 6], self.expected_Data[j + 7], self.expected_Data[j + 8], self.expected_Data[j + 9]]
                j += 10
    ...
    def reportData(self, server):
        ...
        transcript_data = {"testId": self.testId, "testUserId": self.testUserId,
                           ...
                           "print": False}
        transcript_request = requests.post(self.webapi, headers=transcript_header, data=json.dumps(transcript_data),
                                           verify=True)
        ...
        # print("CodingQuestion",codingQuestions)

        actual_question_data = dict()
        q_data =  []
        for d in codingQuestions:
            obtainedMarks = d["obtainedMark"]
            totalMarks = int(d["mark"])
            language = d["language"]
            compile_Status = d["resultStatus"]
            codingSummary = d["codingSummary"]
            if codingSummary is None:
                testcase_Passed = "NA"
            else:
                testcase_Passed = codingSummary.get("testCasePassed")
            total_TestCases = len(d["testCases"])
            timeSpent = d["timeSpent"]
            if obtainedMarks == 0 or obtainedMarks is None:
            ...
            if total_TestCases <= 0:
                TC_Pass_Status = "N/A"
                q_data.append(TC_Pass_Status)
            else:
                TC_Pass_Status = str(testcase_Passed) + "/" + str(total_TestCases)
                q_data.append(TC_Pass_Status)
            q_data.append(timeSpent)
            if codingSummary is None:
                avg_ExecutionTime = "N/A"
                avg_MemoryConsumption = "N/A"
                codeComplexity = "N/A"
                linesOfCode = "N/A"
                q_data.append(avg_ExecutionTime)
                q_data.append(avg_MemoryConsumption)
                q_data.append(codeComplexity)
                q_data.append(linesOfCode)
            else:
                avg_ExecutionTime = d["codingSummary"]["avgTestCaseExecutionTime"]
                avg_MemoryConsumption = d["codingSummary"]["avgMemoryUsage"]
                codeComplexity = d["codingSummary"]["codeComplexity"]
                linesOfCode = d["codingSummary"]["linesOfCode"]
                if avg_ExecutionTime is None:
                    avg_ExecutionTime = "N/A"
                    q_data.append(avg_ExecutionTime)
                else:
                    q_data.append(avg_ExecutionTime)
                if avg_MemoryConsumption is None:
                    avg_MemoryConsumption = "N/A"
                    q_data.append(avg_MemoryConsumption)
                else:
                    q_data.append(avg_MemoryConsumption)
                if codeComplexity is None:
                    codeComplexity = "N/A"
                    q_data.append(codeComplexity)
                else:
                    q_data.append(codeComplexity)
                if linesOfCode is None:
                    linesOfCode = "N/A"
                    q_data.append(linesOfCode)
                else:
                    q_data.append(linesOfCode)
            ...
            q_data = []
        print("actualed_question_data", actual_question_data)
    ...
    def logout(self, server):
        ...
        logout_data = {}
        logout_request = requests.post(self.webapi, headers=crpo_logout_header, data=json.dumps(logout_data), verify=True)
        logout_response = logout_request.json()

# if __name__ == "__main__":
#     trans = CodingQAPerformance()
#     trans.coding_qa_performance(server)
#     trans.login(server)
#     trans.reportData(server)
#     trans.logout(server)